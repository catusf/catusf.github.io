/**
 * phan-shared.js — Reusable file/URL loading for all Phan tools
 *
 * Provides:
 *   PhanFile.createActionBar(container, {onFile, onText, fileLabel, urlLabel})
 *   PhanFile.createUrlModal(onLoad)
 *   PhanFile.loadFromUrl(url, callback)
 *   PhanFile.autoLoadFromParams(callback)
 */
window.PhanFile = (function() {
  'use strict';
  var modalEl = null;
  var modalInput = null;
  var modalError = null;
  ...
  function ensureModal() {
    ...
    modalEl.innerHTML =
      '<div class="modal">' +
        ...
        '<input type="text" class="input" id="phan-url-input" placeholder="https://example.com/family.ged" style="width:100%;margin-bottom:8px" autocomplete="off" spellcheck="false">' +
        '<div id="phan-url-error" style="font-size:13px;color:var(--md-error);min-height:20px"></div>' +
        '<div class="modal__actions">' +
          '<button class="btn btn-text" id="phan-url-cancel">Cancel</button>' +
          '<button class="btn btn-filled" id="phan-url-confirm"><span class="mi">download</span>Load</button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(modalEl);
    modalInput = document.getElementById('phan-url-input');
    modalError = document.getElementById('phan-url-error');

    document.getElementById('phan-url-cancel').addEventListener('click', hideModal);
    modalEl.addEventListener('click', function(e) { if (e.target === modalEl) hideModal(); });
    document.getElementById('phan-url-confirm').addEventListener('click', function() {
  ...
  function createActionBar(container, opts) {
    ...
    bar.innerHTML =
      '<button class="btn btn-tonal" id="phan-file-btn"><span class="mi">folder_open</span>Open file</button>' +
      '<button class="btn btn-tonal" id="phan-url-btn"><span class="mi">link</span>Load from URL</button>' +
      '<input type="file" id="phan-file-input" accept="' + accept + '" style="display:none">';
    container.insertBefore(bar, container.firstChild);

    var fileInput = bar.querySelector('#phan-file-input');
    bar.querySelector('#phan-file-btn').addEventListener('click', function() { fileInput.click(); });
    bar.querySelector('#phan-url-btn').addEventListener('click', function() {
